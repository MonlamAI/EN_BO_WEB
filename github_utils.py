...
def create_github_repo(path, org_name, token, private=False, description=""):
    org = _get_openpecha_data_org(org_name, token)
...
def create_local_repo(path, remote_url, org, token):
    if (path / ".git").is_dir():
        ...
        repo.config_writer().set_value("user", "name", "jungtop").release()
        repo.config_writer().set_value("user", "email", "tenzintopjor27@gmail.com").release()
        return repo
...
def create_github_issue(repo_name,org=None,token=None):
    if not token:
